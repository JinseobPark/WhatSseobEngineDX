...
bool SystemClass::Update()
{
	...
	// 입력 프레임 처리를 수행합니다
	//if (!m_Input->Update()) return false;
	...
	m_Position->MoveUp(m_Input->IsUpPressed());
	m_Position->MoveDown(m_Input->IsDownPressed());
	m_Position->MoveForward(m_Input->IsForwardPressed());
	m_Position->MoveBack(m_Input->IsPicked(DIK_E));
	...
	result = m_Graphics->Frame(dt, position);
	if (!result)
	{
		return false;
	}
