...
bool VerticalBlurShaderClass::Initialize(ID3D11Device* device, HWND hwnd)
{
	...
	return InitializeShader(device, hwnd, L"data/hlsl/verticalblurvs.hlsl", L"data/hlsl/verticalblurps.hlsl");
}
...
bool VerticalBlurShaderClass::InitializeShader(ID3D11Device* device, HWND hwnd, const WCHAR* vsFilename, const WCHAR* psFilename)
{
	HRESULT result;
	...
	result = D3DCompileFromFile(vsFilename, NULL, NULL, "VerticalBlurVertexShader", "vs_5_0", D3D10_SHADER_ENABLE_STRICTNESS, 0, &vertexShaderBuffer, &errorMessage);
	if (FAILED(result))
	{
	...
	result = D3DCompileFromFile(psFilename, NULL, NULL, "VerticalBlurPixelShader", "ps_5_0", D3D10_SHADER_ENABLE_STRICTNESS, 0, &pixelShaderBuffer, &errorMessage);
	if (FAILED(result))
	{
		// If the shader failed to compile, show message
		if (errorMessage)
	...
	// Set description of the sizet buffer in the vertex shader.
