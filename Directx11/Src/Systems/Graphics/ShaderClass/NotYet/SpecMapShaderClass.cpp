...
{
	// 정점 및 픽셀 쉐이더를 초기화합니다.
	return InitializeShader(device, hwnd, L"specmap.vs", L"specmap.ps");
}
	...
	XMFLOAT3 cameraPosition, XMFLOAT4 specularColor, float specularPower)
{
	...
	if (!SetShaderParameters(deviceContext, worldMatrix, viewMatrix, projectionMatrix, textureArray, lightDirection, diffuseColor, cameraPosition, specularColor, specularPower))
	{
		return false;
	}
	...
	XMFLOAT3 cameraPosition, XMFLOAT4 specularColor, float specularPower)
{
	...
	// 버텍스 쉐이더에있는 카메라 상수 버퍼의 위치를 ??두 번째 버퍼로 설정합니다.
	bufferNumber = 1;

	// 이제 정점 셰이더에서 행렬 상수 버퍼를 업데이트 된 값으로 설정합니다.
