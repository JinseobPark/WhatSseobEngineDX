#include "stdafx.h"
#include "ShadowMultiShaderClass.h"


ShadowMultiShaderClass::ShadowMultiShaderClass()
{
}


ShadowMultiShaderClass::ShadowMultiShaderClass(const ShadowMultiShaderClass& other)
{
}
...
bool ShadowMultiShaderClass::Initialize(ID3D11Device* device, HWND hwnd)
{
	// 정점 및 픽셀 쉐이더를 초기화합니다.
	return InitializeShader(device, hwnd, L"data/hlsl/shadowmultivs.hlsl", L"data/hlsl/shadowmultiPS.hlsl");
}


void ShadowMultiShaderClass::Shutdown()
{
...
bool ShadowMultiShaderClass::Render(ID3D11DeviceContext* deviceContext, int indexCount, XMMATRIX worldMatrix, XMMATRIX viewMatrix,
...
bool ShadowMultiShaderClass::Render(ID3D11DeviceContext* deviceContext, int indexCount, XMMATRIX worldMatrix, XMMATRIX viewMatrix,
	...
	XMFLOAT4 ambientColor, XMFLOAT4 diffuseColor, XMMATRIX lightViewMatrix2, XMMATRIX lightProjectionMatrix2,
	ID3D11ShaderResourceView* depthMapTexture2, XMFLOAT3 lightPosition2, XMFLOAT4 diffuseColor2,
	XMMATRIX lightViewMatrix3, XMMATRIX lightProjectionMatrix3,
    ID3D11ShaderResourceView* depthMapTexture3, XMFLOAT3 lightDirection,
	XMFLOAT4 diffuseColor3)
{
	// 렌더링에 사용할 셰이더 매개 변수를 설정합니다.
	if (!SetShaderParameters(deviceContext, worldMatrix, viewMatrix, projectionMatrix, lightViewMatrix, lightProjectionMatrix, texture,
		depthMapTexture, lightPosition, ambientColor, diffuseColor, lightViewMatrix2, lightProjectionMatrix2, depthMapTexture2,
		lightPosition2, diffuseColor2, lightViewMatrix3, lightProjectionMatrix3, depthMapTexture3, lightDirection, diffuseColor3))
	{
		return false;
	}
...
bool ShadowMultiShaderClass::InitializeShader(ID3D11Device* device, HWND hwnd, const WCHAR* vsFilename, const WCHAR* psFilename)
{
	HRESULT result;
...
void ShadowMultiShaderClass::ShutdownShader()
{
	// 광원 상수 버퍼를 해제합니다.
	if (m_lightBuffer)
	{
...
void ShadowMultiShaderClass::OutputShaderErrorMessage(ID3D10Blob* errorMessage, HWND hwnd, const WCHAR* shaderFilename)
{
	// 에러 메시지를 출력창에 표시합니다.
...
bool ShadowMultiShaderClass::SetShaderParameters(ID3D11DeviceContext* deviceContext, XMMATRIX worldMatrix, XMMATRIX viewMatrix,
	XMMATRIX projectionMatrix, XMMATRIX lightViewMatrix, XMMATRIX lightProjectionMatrix,
	ID3D11ShaderResourceView* texture, ID3D11ShaderResourceView* depthMapTexture, XMFLOAT3 lightPosition,
...
bool ShadowMultiShaderClass::SetShaderParameters(ID3D11DeviceContext* deviceContext, XMMATRIX worldMatrix, XMMATRIX viewMatrix,
	...
	XMFLOAT4 ambientColor, XMFLOAT4 diffuseColor, XMMATRIX lightViewMatrix2,
	XMMATRIX lightProjectionMatrix2, ID3D11ShaderResourceView* depthMapTexture2, XMFLOAT3 lightPosition2,
	XMFLOAT4 diffuseColor2, XMMATRIX lightViewMatrix3, XMMATRIX lightProjectionMatrix3,
	ID3D11ShaderResourceView* depthMapTexture3, XMFLOAT3 lightDirection,
	XMFLOAT4 diffuseColor3)
{
	// 행렬을 transpose하여 셰이더에서 사용할 수 있게 합니다
...
void ShadowMultiShaderClass::RenderShader(ID3D11DeviceContext* deviceContext, int indexCount)
{
	// 정점 입력 레이아웃을 설정합니다.
