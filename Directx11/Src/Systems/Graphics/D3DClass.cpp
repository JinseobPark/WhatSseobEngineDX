	...
	float screenDepth, float screenNear)
{
	...
	swapChainDesc.OutputWindow = mAPPLICATION->GetHWND();
	...
	// ̉Discard the back buffer contents after presenting.
	...
	// Clear blend state before setting the parameters.
	D3D11_BLEND_DESC blendStateDescription;
	ZeroMemory(&blendStateDescription, sizeof(D3D11_BLEND_DESC));

	//  Set Alpha blending staate
	...
	// Create enable state using the device.
	if (FAILED(m_device->CreateBlendState(&blendStateDescription, &m_alphaEnableBlendingState)))
	{
		return false;
	}

	// set blending disable state
	blendStateDescription.RenderTarget[0].BlendEnable = FALSE;

	// Create disable state using the device.
