...
{
	//Add model vector
	mGRAPHICS->m_models.push_back(this);
}

Model::~Model()
{
	//Delete model vector
	mGRAPHICS->m_models.erase(std::find(mGRAPHICS->m_models.begin(), mGRAPHICS->m_models.end(), this));
}
...
ID3D11ShaderResourceView* Model::GetTexture(TextureType type)
{
...
void Model::InitializeTextureManager()
{
...
void Model::AddTextures(const WCHAR* filename, TextureType type)
{
