...
{
	// 정점 및 픽셀 쉐이더를 초기화합니다.
	return InitializeShader(device, hwnd, L"data/hlsl/translateVS.hlsl", L"data/hlsl/translatePS.hlsl");
}
	...
	XMMATRIX projectionMatrix, ID3D11ShaderResourceView* texture, float translation)
{
	...
	if (!SetShaderParameters(deviceContext, worldMatrix, viewMatrix, projectionMatrix, texture, translation))
	{
		return false;
	}
...
bool TranslateShaderClass::InitializeShader(ID3D11Device* device, HWND hwnd, WCHAR* vsFilename, WCHAR* psFilename)
{
	...
	result = D3DCompileFromFile(psFilename, NULL, NULL, "TranslatePixelShader", "ps_5_0", D3D10_SHADER_ENABLE_STRICTNESS, 0, &pixelShaderBuffer, &errorMessage);
	if (FAILED(result))
	{
		// 셰이더 컴파일 실패시 오류메시지를 출력합니다.
		if (errorMessage)
	...
	XMMATRIX projectionMatrix, ID3D11ShaderResourceView* texture, float translation)
{
	...
	// 텍스처 변환 상수 버퍼를 잠글 수 있도록 기록합니다.
	if (FAILED(deviceContext->Map(m_translateBuffer, 0, D3D11_MAP_WRITE_DISCARD, 0, &mappedResource)))
	{
		return false;
	}
