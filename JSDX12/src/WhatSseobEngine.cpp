...
void Engine::LoadTextures()
{
	...
	bricksTex2->Filename = L"Textures/bricks2.dds";
	ThrowIfFailed(DirectX::CreateDDSTextureFromFile12(md3dDevice.Get(),
		mCommandList.Get(), bricksTex2->Filename.c_str(),
		bricksTex2->Resource, bricksTex2->UploadHeap));

	auto bricksTex3 = std::make_unique<Texture>();
	bricksTex3->Name = "bricksTex3";
	bricksTex3->Filename = L"Textures/bricks3.dds";
	ThrowIfFailed(DirectX::CreateDDSTextureFromFile12(md3dDevice.Get(),
		mCommandList.Get(), bricksTex3->Filename.c_str(),
		bricksTex3->Resource, bricksTex3->UploadHeap));
	...
	mTextures[bricksTex2->Name] = std::move(bricksTex2);
	mTextures[bricksTex3->Name] = std::move(bricksTex3);
...
void Engine::BuildRenderItems()
{
	...
	XMStoreFloat4x4(&carRitem2->World, XMMatrixScaling(1.0f, 1.0f, 1.0f)* XMMatrixTranslation(-5.0f,5.0f, 0.0f));
	XMStoreFloat4x4(&carRitem2->TexTransform, XMMatrixScaling(1.0f, 1.0f, 1.0f));
	carRitem2->ObjCBIndex = objCBIndex++;
	carRitem2->Mat = mMaterials["bricks2"].get();
	...
	XMStoreFloat4x4(&carRitem->World, XMMatrixScaling(1.0f, 1.0f, 1.0f) * XMMatrixTranslation(5.0f, 10.0f, 0.0f));
	XMStoreFloat4x4(&carRitem->TexTransform, XMMatrixScaling(2.0f, 2.0f, 1.0f));
	carRitem->ObjCBIndex = objCBIndex++;
	carRitem->Mat = mMaterials["tile0"].get();
